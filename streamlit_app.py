...
<style>
    ...
    .alert-high {
        ...
        padding: 1rem;
        border-radius: 0.5rem;
        color: #d00000; /* Dark red text for visibility */
    }
    .alert-high h3 {
        color: #d00000 !important; /* Ensure header is dark red */
    }
    .alert-high p {
        color: #d00000 !important; /* Ensure paragraph text is dark red */
    }
    .alert-high strong {
        color: #d00000 !important; /* Ensure strong text is dark red */
    }
    .alert-moderate {
        background-color: #ffe6b3; /* Lighter orange */
        border-left: 4px solid #ff9900; /* Bright orange */
        padding: 1rem;
        border-radius: 0.5rem;
        color: #cc7000; /* Dark orange text for visibility */
    }
    .alert-moderate h3 {
    ...
    .alert-moderate strong {
        color: #cc7000 !important;
    }
    .alert-low {
        background-color: #ccffcc; /* Lighter green */
        border-left: 4px solid #33cc33; /* Bright green */
        padding: 1rem;
        border-radius: 0.5rem;
        color: #006600; /* Dark green text for visibility */
    }
    .alert-low h3 {
...
class TwilioIntegration:
    ...
    def _initialize_client(self):
        ...
        if TWILIO_AVAILABLE and self.account_sid and self.auth_token:
            try:
                self.client = Client(self.account_sid, self.auth_token)
                st.success(f"✅ Twilio client initialized with SID: {self.account_sid[:8]}...")
            except Exception as e:
                st.error(f"Twilio initialization error: {e}")
                self.client = None
        else:
            st.warning("⚠️ Twilio credentials not fully provided—running in demo mode.")
            self.client = None

    def update_credentials(self, sid, token, from_phone, to_phone):
        """Update Twilio credentials and reinitialize client."""
...
def main():
    ...
    **Real-time panic attack risk assessment for autistic children**
    ...
    with st.sidebar.expander("📞 Twilio Configuration"):
        ...
        if st.button("Update Twilio Config"):
            if twilio_sid and twilio_token and twilio_from and twilio_to:
                twilio.update_credentials(twilio_sid, twilio_token, twilio_from, twilio_to)
                st.experimental_rerun()  # Force a rerun to apply new credentials
            else:
                st.warning("Please provide all Twilio credentials")
        ...
        try:
            ...
            if panic_probability > 0.7:
                ...
                <div class="alert-high">
                    <h3>🚨 HIGH PANIC RISK DETECTED</h3>
                    <p><strong>Probability: {panic_probability:.1%}</strong></p>
                    <p>Immediate intervention recommended. Emergency alert triggered automatically.</p>
                </div>
                """, unsafe_allow_html=True)
                
                # Check if Twilio is ready
                if not twilio.client:
                    st.warning("⚠️ Twilio not configured—running in demo mode. Update credentials in the sidebar!")
                
                # Automatic emergency call trigger with cooldown
                current_time = datetime(2025, 9, 26, 3, 10)  # 03:10 AM IST, September 26, 2025
                if 'last_alert_time' not in st.session_state:
                    st.session_state.last_alert_time = None
                last_alert = st.session_state.last_alert_time
                cooldown = timedelta(minutes=5)  # Prevent repeated calls
                ...
                if last_alert is None or (current_time - last_alert) > cooldown:
                    ...
                    st.info(f"⏳ Cooldown active—next alert possible at {last_alert + cooldown}")
    ...
    with col2:
        ...
        current_time = datetime(2025, 9, 26, 3, 10)  # 03:10 AM IST, September 26, 2025
        st.info(f"Last updated: {current_time.strftime('%H:%M:%S %d/%m/%Y IST')}")
    ...
    fig_trend.add_hline(y=0.7, line_dash="dash", line_color="#ff3333", 
                       annotation_text="High Risk Threshold")
    fig_trend.add_hline(y=0.4, line_dash="dash", line_color="#ff9900", 
                       annotation_text="Moderate Risk Threshold")
